		...
		if (infs.is_open()){
			while (getline(infs,line)){
				...
				//try{
					getline(sin, strTime, ' ');
					getline(sin, country, ' ');
					getline(sin, strNumber, ' ');
					getline(sin, name);
					name = trim(name);
					
					
					//cout << strTime << " " << country << " " << strNumber << " " << name << endl;
				...
				//cout << (priorValidInfo && anotherValidInfo) << " CHECK " << endl;
				//cout << validInfo << "VALID INFO" << endl;
